...
{
	//while( psz && *psz == _T(' ') && *psz == 13 && *psz == 10 ) psz++;
...
AFX_EXT_API CString _tagXMLAttr::GetXML( LPDISP_OPT opt /*= &optDefault*/ )
{
	...
	//os << (LPCTSTR)name << _T("='") << (LPCTSTR)value << _T("' ");
	
	os << (LPCTSTR)name << _T("=") << (char)opt->value_quotation_mark
		<< (LPCTSTR)(opt->reference_value&&opt->entitys?opt->entitys->Entity2Ref(value):value) 
		<< (char)opt->value_quotation_mark << _T(" ");
	return os.str().c_str();
}
